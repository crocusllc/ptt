...
def create_app():
    ...
    def file_download():
        ...
        if query_fields != "*":
            if ("start_date" in fields and fields['start_date'] != '') and ("end_date" in fields and fields['end_date'] != ''):
                date_condition = f"((TO_DATE(PGP_SYM_DECRYPT('start_date','{key_encrypt}'::text), 'MM/DD/YYYY') >= '{fields['start_date']}'::date AND TO_DATE(PGP_SYM_DECRYPT('end_date','{key_encrypt}'::text), 'MM/DD/YYYY') <= '{fields['start_date']}'::date) OR (TO_DATE(PGP_SYM_DECRYPT('start_date','{key_encrypt}'::text), 'MM/DD/YYYY') >= '{fields['end_date']}'::date AND TO_DATE(PGP_SYM_DECRYPT('end_date','{key_encrypt}'::text), 'MM/DD/YYYY') <= '{fields['end_date']}'::date))"
            elif ("start_date" in fields and fields['start_date'] != ''):
                date_condition = f"(TO_DATE(PGP_SYM_DECRYPT('start_date','{key_encrypt}'::text), 'MM/DD/YYYY') >= '{fields['start_date']}'::date)"
            elif ("end_date" in fields and fields['start_date'] != ''):
                date_condition = f"(TO_DATE(PGP_SYM_DECRYPT('end_date','{key_encrypt}'::text), 'MM/DD/YYYY') <= '{fields['end_date']}'::date)"
    ...
    def delete_record():
        ...
        if (data != {}):
            if 'table_name' in data and 'id' in data:
                ...
                query = f"DELETE FROM {table} WHERE clinical_id={id};"
                cur.execute(query)
                conn.commit()
